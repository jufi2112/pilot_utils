...
if __name__ == '__main__':
    ...
    if args.watched is None:
        watched_fpath = osp.join(get_package_root(), 'pilot_utils', 'azf_trainer', 'data', 'bookmarks.json')
    else:
        watched_fpath = args.watched
    if args.done is None:
        done_fpath = osp.join(get_package_root(), 'pilot_utils', 'azf_trainer', 'data', 'hidden.json')
    else:
