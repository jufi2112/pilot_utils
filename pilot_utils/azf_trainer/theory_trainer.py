...
from pilot_utils.azf_trainer.src import AZFTrainingController, AZFTrainerModel, QuestionMode
...
if __name__ == '__main__':
    ...
    if args.questions is None:
        questions_fpath = osp.join('data', 'questionnaire.json')
    else:
        questions_fpath = args.questions
    if args.watched is None:
        watched_fpath = osp.join('data', 'watched.json')
        if not osp.exists(watched_fpath):
            watched_fpath = None
    else:
        watched_fpath = args.watched
    if args.done is None:
        done_fpath = osp.join('data', 'done.json')
        if not osp.exists(done_fpath):
            done_fpath = None
    else:
